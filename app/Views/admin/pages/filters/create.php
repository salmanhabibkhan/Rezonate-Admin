            ...
            <div class="col-md-8">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title"><?= lang('Admin.create_filter') ?></h3>
                    </div>
                    <div class="card-body">
                        <form action="<?= base_url('admin/filters/store') ?>" method="post" enctype="multipart/form-data" id="create_blog_form">

                            <div class="mb-3">
                                <label for="title" class="form-label"><?= lang('Admin.name') ?></label>
                                <input type="text" class="form-control" name="name" placeholder="<?= lang('Admin.enter_name') ?>" value="<?= old('name'); ?>">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="link" placeholder="<?= lang('Admin.enter_link') ?>" value="<?= old('link'); ?>">
                                <?= !empty($validation->getError('link')) ? "<span class='text-danger'>" . $validation->getError('link') . "</span> " : ''; ?>
                            </div>
                            <div class="mb-3">
        ...
        $("#create_blog_form").validate({
            ...
            messages: {
                name: {
                    ...
                    minlength: "<?= lang('Admin.validation_name_minlength') ?>",
                    maxlength: "<?= lang('Admin.validation_name_maxlength') ?>"
                },
                link: {
                    required: "<?= lang('Admin.validation_link_required') ?>"
                }
            }
        });
    });
