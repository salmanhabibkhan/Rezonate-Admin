            ...
            <div class="col-md-8">
                ...
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title"><?= lang('Admin.edit_blog') ?></h3>
                    </div>
                    <div class="card-body">
                        <form action="<?= base_url('admin/blogs/update/' . $blog['id']) ?>" method="post" enctype="multipart/form-data" id="edit_blog_form">

                            <div class="mb-3">
                                <label for="title" class="form-label"><?= lang('Admin.title') ?></label>
                                <input type="text" class="form-control" name="title" placeholder="<?= lang('Admin.enter_title') ?>" value="<?= $blog['title']; ?>">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="category" class="form-control">
                                    ...
                                    <?php foreach (BLOG_CATEGORIES as $key => $category) : ?>
                                        <option value="<?= $key ?>" <?= ($key == $blog['category']) ? 'selected' : ''; ?>><?= $category ?></option>
                                    <?php endforeach ?>
                                </select>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <?= !empty($validation->getError('thumbnail')) ? "<span class='text-danger'>" . $validation->getError('thumbnail') . "</span> " : '' ?>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="active" class="form-label"><?= lang('Admin.status') ?></label>
                                    <select name="active" class="form-control">
                                        <option value="1" <?= ($blog['active'] == 1) ? 'selected' : ''; ?>><?= lang('Admin.active') ?></option>
                                        <option value="0" <?= ($blog['active'] == 0) ? 'selected' : ''; ?>><?= lang('Admin.inactive') ?></option>
                                    </select>
                                    <?= !empty($validation->getError('active')) ? "<span class='text-danger'>" . $validation->getError('active') . "</span> " : '' ?>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                <label for="tags" class="form-label"><?= lang('Admin.blog_tags') ?></label>
                                <input type="text" name="tags" id="tags" class="form-control" placeholder="<?= lang('Admin.enter_tags') ?>" value="<?= implode(', ', $tags); ?>">
                                <?= !empty($validation->getError('tags')) ? "<span class='text-danger'>" . $validation->getError('tags') . "</span> " : '' ?>
                            </div>
    ...
    $(document).ready(function() {
        ...
        $("#edit_blog_form").validate({
            ...
            rules: {
                ...
                thumbnail: {
                    // Add any additional rules for the file if needed
                },
                active: {
                    required: true
                },
            ...
            messages: {
                ...
                thumbnail: {
                    // Add messages for the file if needed
                },
                active: {
