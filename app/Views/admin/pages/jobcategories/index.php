                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="example1" class="table table-hover table-bordered admin-side-tables table-striped">
                                ...
                                <tbody>
                                    <?php  $startIndex = ($pager->getCurrentPage() - 1) * $perPage; ;?>
                                    <?php foreach ($categories as $key => $category): ?>
                                    <tr>
                                        <td><?= $startIndex + $key + 1 ?></td>
                                        <td><?= $category['name'] ?></td>
                                        <td>
                                            <a href="<?= base_url('admin/job-categories/edit/'.$category['id']) ?>" class="btn btn-sm btn-primary btn-wave">
                                                <i class="fa fa-pen mr-2 text-light"></i> <?= lang('Admin.edit') ?>
                                            </a>
                                            <?php if($category['id'] > 30): ?>
                                                <a href="<?= base_url('admin/job-categories/delete/'.$category['id']) ?>" onclick="return confirm('<?= lang('Admin.confirm_delete') ?>')" class="btn btn-sm btn-danger btn-wave">
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="<?= base_url('admin/job-categories/store') ?>" method="POST">
