        ...
        <div class="container py-lg-5 my-lg-5">
            <div class="row mb-4 mb-sm-5">
                <div class="col-lg-7 col-md-9 text-center text-sm-start">
                    <h1 class="text-white lh-base"><span class='fw-light'><?= esc(lang('Web.over')) ?></span> 1,500 <span class='fw-light'><?= esc(lang('Web.curated')) ?></span> <?= esc(lang('Web.design_website_templates')) ?></h1> <!-- Translate "Over 1,500 curated Design & Website templates" -->
                    <h2 class="h5 text-white fw-light"><?= esc(lang('Web.high_quality_items')) ?></h2> <!-- Translate "High quality items created by our global community" -->
                </div>
            </div>
    ...
    <div class="card px-lg-2 border-0 shadow-sm">
        <div class="card-body px-4 pt-5 pb-4">
            <div class="d-flex justify-content-between border-bottom mb-3">
                <div class="text-left">
                    <h2 class="h3"><?= esc(lang('Web.discover_featured_products')) ?></h2> <!-- Translate "Discover featured products" -->
                    <p class="text-muted"><?= esc(lang('Web.handpicked_items')) ?></p> <!-- Translate "Every week we hand-pick some of the best items from our collection" -->
                </div>
                <div>
                    <a href="<?= site_url('products/create-product') ?>" class="btn btn-primary m-2"><?= esc(lang('Web.add_new_product')) ?></a> <!-- Translate "Add New Product" -->
                    <a href="<?= site_url('products/my-products') ?>" class="btn btn-primary m-2">
                    ...
                    <div class="carousel-item active">
                        <div class="row">
                            <?php foreach ($sliderproducts as $key => $sliderproduct) : ?>
                                <div class="col-md-3">
                                    <img class="d-block w-100" src="<?= $sliderproduct['images'][0]['image']?>" alt="Image <?= $key?>">
                                </div>
                            <?php endforeach ?>
                        </div>
                    </div>
